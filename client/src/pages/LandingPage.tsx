...
function NativeAuthButtons() {
  ...
  const handleGoogleAuth = async () => {
    ...
    try {
      // Start OAuth flow - this returns the URL to redirect to
      const result = await signIn.authenticateWithRedirect({
        strategy: 'oauth_google',
        redirectUrl: 'eidola://oauth-callback',
        redirectUrlComplete: 'eidola://oauth-complete'
      })

      // If authenticateWithRedirect returns a URL, open it in system browser
      // The actual redirect happens via the signIn object
      console.log('[NativeAuthButtons] OAuth redirect initiated', result)
    } catch (error) {
  ...
  const handleAppleAuth = async () => {
    ...
    try {
      ...
      console.error('[NativeAuthButtons] Apple OAuth error:', error)
    }
  }
